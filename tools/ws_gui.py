...
COMMAND_GUIDE = (
    ...
    ("TAP_PERCENT", "TAP_PERCENT:50,75", "Bấm theo phần trăm chiều rộng và chiều cao."),
    ("SWIPE", "SWIPE:540,1500,540,500,350", "Vuốt từ A đến B trong thời gian mili giây."),
    ("SWIPE_PERCENT", "SWIPE_PERCENT:50,80,50,25,450", "Vuốt theo phần trăm để dùng chung nhiều độ phân giải."),
    ("WAIT", "WAIT:Văn bản", "Chờ node có text hoặc mô tả xuất hiện."),
    ("WAIT_TIMEOUT", "WAIT_TIMEOUT:Văn bản|10", "Chờ text tối đa số giây chỉ định."),
    ("WAIT_IMG", "WAIT_IMG:tên_mẫu", "Chờ ảnh mẫu xuất hiện trên màn hình."),
    ("WAIT_IMG_TIMEOUT", "WAIT_IMG_TIMEOUT:tên_mẫu|10", "Chờ ảnh tối đa 10 giây."),
    ("WAIT_ANY_IMG", "WAIT_ANY_IMG:nhan,xem,batdau|30|HET_GIO", "Chờ ảnh xuất hiện trước và lưu vào LAST_IMAGE."),
    ("CLICK_IMG", "CLICK_IMG:tên_mẫu", "Tìm và bấm vào ảnh mẫu."),
    ("CLICK_IMG_TIMEOUT", "CLICK_IMG_TIMEOUT:tên_mẫu|10", "Tìm và bấm ảnh, quá hạn thì báo lỗi hoặc phục hồi."),
    ("CHECKPOINT", "CHECKPOINT:SAVE", "Lưu điểm tiếp tục an toàn; CHECKPOINT:CLEAR để xóa."),
    ("REQUIRE_DEVICE", "REQUIRE_DEVICE:UNLOCKED,CAPTURE,NETWORK|PHUC_HOI", "Rẽ nhánh nếu điều kiện thiết bị chưa đạt."),
    ...
    ("SET", "SET:SO_LAN=0", "Tạo hoặc gán giá trị biến."),
    ("INC", "INC:SO_LAN,1", "Tăng giá trị biến."),
    ("IF_VAR", "IF_VAR:SO_LAN>=10|KET_THUC", "So sánh biến rồi rẽ nhánh."),
    ("ON_ERROR", "ON_ERROR:PHUC_HOI", "Khi có lỗi, nhảy tới nhãn phục hồi; dùng ON_ERROR:OFF để tắt."),
