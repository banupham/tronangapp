...
class GenericAccessibilityService : AccessibilityService() {
    ...
    fun clickText(requestedText: String): Boolean {
        ...
        // Fast path: use the index generated by the latest Accessibility event.
        if (clickFromLiveIndex(expected)) return true

        // Fallback in case an indexed AccessibilityNodeInfo became stale.
        val root = rootInActiveWindow ?: return false
    ...
    private fun handleJsonRemoteCommand(rawJson: String) {
        ...
        when (json.optString("cmd").trim().lowercase()) {
            ...
            "stop" -> mainHandler.post { stopWorkflow() }
            ...
            "run", "workflow" -> {
                ...
                if (script.isBlank()) {
                    ...
                    mainHandler.post { runWorkflow(script) }
                }
            }
            ...
            "image_find" -> {
                ...
                mainHandler.post {
                    runWorkflow((if (click) "CLICK_IMG:" else "WAIT_IMG:") + name)
                }
            }
